...
const double _stallCurrent = 8.0;
...
const byte servoPin[nbServos] = {9, 8, 7, 4};                 //{4,7,2,8};  // digital pins (not necessarily ~pwm)
//const bool inversion[nbServos] = {false, false, false, false};//{true, true, true, true};
const bool inversion[nbServos] = {true, true, true, true}; // parameter to change if a servo goes the wrong way
int OldSerialValue[nbServos] = {0, 0, 0, 0};
int intendedDegrees[nbServos] = {0, 0, 0, 0};

// servo span:
int servoHomeDegrees[nbServos] = {5, 15, 5, 5};     // will be updated with the initial pressure measurement
int servoEndDegrees[nbServos] = {175, 175, 175, 175}; // leftthigh, rightthigh, leftside, rightside
int currentServoIndex = 0;

int servoNativeDegrees[nbServos] = {270, 270, 270, 270};
int servoRotationOffsetDegrees[nbServos] = {0, 0, 0, 0}; //{90,90,90,90};
